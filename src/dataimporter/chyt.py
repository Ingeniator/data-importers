"""CHYT (ClickHouse over YTsaurus) read adapter — search queries only."""
...
def _chyt_params(ds: Datasource) -> dict:
    params: dict = {}
    if ds.database:
        params["database"] = ds.database  # CHYT clique alias, e.g. *ch_public
    if ds.user:
        params["user"] = ds.user
    if ds.password:
...
async def search_logs_chyt(
    ...
    try:
        ...
        logger.error("chyt_search_failed", error=str(e))
        return []
